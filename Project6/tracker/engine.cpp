...
void Engine::draw() const {
  ...
  if(Health::getInstance().getHealthBar() <= 0){
	  hud.hudEnd(false);
   }
...
void Engine::checkForCollisions() {
  ...
  if(strategy->execute(*player, *tower)){
    end = true;
  }
    ...
    while ( SDL_PollEvent(&event) ) {
      ...
      if(event.type == SDL_KEYDOWN) {
        ...
        if ( keystate[SDL_SCANCODE_R] ) {
          ...
          player->reset();
          for(auto* s: smart){s->reset();}
          end = false;
          player->godMode();
        }

        if (keystate[SDL_SCANCODE_G] ) {
          static_cast<ShootingSprite*>(player)->godMode();
        }
